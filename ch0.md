...
那么AIMD需要什么呢？　假设一个有用的轨迹包含大约$10^M$分子动力学步数，即我们需要$10^M$次电子结构计算来产生一个轨迹. 此外，为了对不同的初始条件做平均，假设我们需要$10^n$个独立轨迹，那么我们总共需要做$10^{M+n}$次AIMD运算.　最后，假设每一次单点电子结构计算需要设计全局势能表面,　且一个时间步骤需要大致相同的CPU时间量.(it is assumed that each single-point electronic structure calculation needed to devise the global potential energy surface and one time step requires roughly the same amount of CPU time.) 基于这种真正简单的数量级估计, AIMD计算相对于计算全局势能面的计算优势达到了$10^{3N-6-M-n}$. 重要的一点是：对于给定的统计精确度(即$M,n$为固定值而不依赖于$N$)和给定的电子结构方法“即时”分子动力学方法的计算优势随系统尺寸以$10^N$增长.　因此, 如果系统足够大且足够复杂，那么Car–Parrinello方法总是跑赢传统的"三步"方法.　相反，如果对于给定的系统的粒子数$N\sim 10^{M+n}$，系统足够小以至于全局势能面可以计算出来并参数化，那么事先计算全局势能面然后，在不需要额外计算成本的情况下运行多次经典轨迹总是值得的.
...
​	推导经典分子动力学的下一步是将原子核近似为经典点粒子．给定描述所有原子核的一个量子力学波动方程(说的是方程$(7)$吗？)，在TDSCF框架下如何才能实现这个目标呢？　通常，从量子力学中提取经典力学的一个著名思路(route)是: 首先，以实振幅因子$A$和实相位因子$S$来重写相应的波函数
$$
...
​	然而，原子核波函数仍然出现在电子的TDSCF方程，并且，为了方便，我们不得不用原子核位置来取代原子核波函数．在这种情形，通过在$\hbar \to 0$极限用一个$\delta$函数之乘积$\prod_I \delta(R_I-R_I(t))$取代方程$(6)$中的原子核密度$|\chi(\{R_I\};t)|$，经典还原能够实现（这些$\delta$函数之中心位于经典原子核的瞬时位置$\{R_I(t)\}$，正如方程$(15)$给出）.　这种取代给出了required平均值：
$$
...
随着经典原子核通过方程$(15)$传播，该电子波函数自洽地演化．注意，此时$H_e$ 依赖于原子核位置$\{\R_I(t)\}$的方式是：$\{\R_I(t)\}$ 作为$V_{n-e}(\{r_i\},\{ \R_I(t)\})$的参数，进而$\Psi$也以同样的方式依赖于$\{\R_I(t)\}$. 这意味着无论是经典自由度对量子自由度的反馈，还是量子自由度对经典自由度的反馈都被合并了起来． (这与＂经典路径＂或者Mott非自洽场动力学方法不一致).[650,651]

​	依赖于方程$(15)$和$(18)$的方法有时被称为Ehrenfest分子动力学，以纪念Ehrenfest．正是他首先提出如何由Shrödinger方程推导牛顿经典动力学这一问题[174]．在当前情形这一方法导致混合方法，因为只有原子核被强制当做经典例子，而电子仍然作为量子粒子处理．

​	虽然依赖于时间的自洽场(TDSCF)方法 underlying Ehrenfest 分子动力学是一个平均场理论，但电子态之间的跃迁也被包含在了这个机制中．为了明白这一点，展开电子波函数$\Psi$ (与方程$(5)$中的总波函数$\Phi$做对照)，以多个电子态(或行列式$\Psi_k$)表示:
$$
\Psi(\{r_i\},\{R_I\};t) = \sum_{k=0}^\infty c_k(t)\Psi_k(\{r_i\};\{R_I\})
$$
其中$\{c_k (t)\}$为复系数． 在此情形，系数的模方$\{|c_k(t)|^2\}$ (满足条件$\sum_k |c_k|^2 =1$)明确地描述不同量子态$\{k\}$的布居数的演化，而量子干涉则由$\{c_k^* c_{l\neq k}\}$给出． 对于基函数$\{\Psi_k\}$，一种可能的*选择*就是：由解不依赖于时间的电子Shrödinger方程 (L:由不依赖时间的薛定谔方程得到定态波函数，即驻波．)
$$
H_e(\{r_i\};\{R_I\})\Psi_k = E_k(\{R_I\})\Psi_k(\{r_i\};\{R_I\})
$$
而得的绝热基,　其中$\{R_I\}$ 是$t$时刻由方程$(15)$而得出的瞬时核位置．以展开系数$\{c_k\}$表示的实际运动方程在2.2节给出．

​	到这一步还可以做进一步简化：根据式子$(20)$，通过**限制**总电子波函数$\Psi$为$H_e$在每一个时刻的基态波函数$\Psi_0$，且$(19)$式中的$c_0$满足$|c_0|^2 \equiv 1$.　一般而言，**如果基态$\Psi_0$和第一激发态$\Psi_1$之间的能量差在各处(everywhere)都比热能$k_BT$大**，那么这将是一个很好的近似．在这个极限下，原子核将依据方程$(15)$在单个势能面

$$
V_e^E = \int d{\bf r} \Psi_0^* H_e \Psi_0 \equiv E_0(\{{\bf R}_I\}).
$$

上运动, 而$E_0$可以通过求解仅对基态的不依赖于时间的薛定谔方程$(20)$式

$$
H_e \Psi_0 = E_0 \Psi_0
$$

而计算出来.　方程$(21)​$就导致了恒等式$V_e^E \equiv E_0​$,也就是说，在上述极限下，Ehrenfest势能和基态Born-Oppenheimer势能是一样的！（L:这两个势都是$\{{\bf R}_I\}​$的函数．）

​	由于这一结果(observation),将生成核动力学的任务与计算势能面分离开是可信的．第一步，对于多个核构型，通过解方程$(22)$计算出$E_0$. 第二步，将这些数据点拟合出一个分析泛函，用于给出全局势能面．第三步，在这个势能面上，对多个不同的初始条件求解牛顿运动方程$(16)$，从而产生一群经典运动轨迹．　简而言之，这就是在全局势能面上的**经典轨迹计算**的基础．

​	正如引言已提到，当活跃的原子核自由度增加时，这些方法会遇到＂维度瓶颈＂困难．为了摆脱这一困境，一个传统方法就是对全局势能面做近似----以如下这些多体贡献的截断的展开式来表示：

$$
V_e^E \approx V_e^{\text{approx}}(\{{\bf R}_I\}) = \sum_{I=1}^N v_1 ({\bf R}_I) + \sum_{I<J}^N v_2({\bf R}_I,{\bf R}_J) \\
+ \sum_{I<J<K}^N v_3({\bf R}_I,{\bf R}_J,{\bf R}_K) + ...
$$

在这个阶段，用相互作用势能$\{v_n\}$ 来取代电子自由度，且在运动方程中电子自由度不具有明确的自由度(and are not featured as explicit degrees of freedom in the equations of motion)． 因此，一旦这相互作用势$\{v_n\}$确定下来，这个混合的量子／经典问题就约化为纯经典力学问题．　经典分子动力学
$$
M_I \frac{d^2}{dt^2}R_I(t) = -\nabla_I V_e^{\text{approx}}(\{R_I(t)\})
$$
就依赖与这个关键思想，且其中只考虑了两体相互作用势$v_2$和三体相互作用势$v_3$，尽管已经有包含诸如极化等非加性相互作用的更加复杂的模型［Isidro,etc］．这相当于一个巨大的简化并且去掉了维度瓶颈，因为全局势能面是从众多可加的两体和三体（统称"**少体**"（few-body）)贡献之和构建出来的．这里要付出了两个代价：其一，引入了一个很严格的近似；其二，基本上将各种化学转化排除在了计算机模拟领域之外．　



​	作为这个推导的一个结果，**经典分子动力学的几个基本假设**变得清晰了：电子们都绝热地跟随这经典原子核的运动并且可以积分掉，以致原子核在一个单Born-Oppenheimer势能面上演化．这个单Born-Oppenheimer势能面一般由电子基态给出，但这不是**必然的**（但这不是**总是如此**）．单BO势能面通常用少体相互作用来近似．